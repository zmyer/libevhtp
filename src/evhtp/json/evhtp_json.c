...
static int             j_compare_(evhtp_json *, evhtp_json *, evhtp_json_key_filtercb);
static int             j_array_to_buffer_(evhtp_json  *, struct __jbuf *);
static int             j_number_to_buffer_(evhtp_json *, struct __jbuf *);
static int             j_object_to_buffer_(evhtp_json *, struct __jbuf *);

struct evhtp_json_s {
    evhtp_json_vtype type;      /**< the data type */
    union {
        evhtp_keyval * object;  /**< object data type ("{}") */
        evhtp_array  * array;   /**< array data type ("[]") */
        char         * string;  /**< string data type ("<str>") */
        unsigned int   number;  /**< number data type (3) */
        bool           boolean; /**< boolean data type (true / false) */
    };

    size_t slen;                /**< size of this data type,
                                 * 'object'  == number of key/value's in an object
                                 * 'array'   == number of entries in the array
                                 * 'string'  == the length of the string minus '\0'
                                 * 'number'  == N/A
                                 * 'boolean' == N/A
                                 */
    void (* freefn)(void *);    /**< data type free function */
};
...
j_object_new_(void) {
    ...
    js->object = evhtp_keyval_new(10);
    js->freefn = (void (*))evhtp_keyval_free;
...
j_array_new_(void) {
    ...
    js->array  = evhtp_array_new();
    js->freefn = (void (*))evhtp_array_free;
...
j_object_add_(evhtp_json * dst,
    ...
    if (!evhtp_keyval_add(dst->object, key, val, (void (*))evhtp_json_free)) {
        return -1;
    }
...
static int
j_object_add_klen_(evhtp_json * dst,
                   const char * key,
                   size_t       klen,
                   evhtp_json * val) {
    if (evhtp_unlikely(j_type_(dst) != evhtp_json_vtype_object)) {
    ...
    if (!evhtp_keyval_add_wklen(dst->object,
                                key, klen, val,
                                (void (*))evhtp_json_free)) {
        return -1;
    }
...
j_array_add_(evhtp_json * dst, evhtp_json * src) {
    if (evhtp_unlikely(j_type_(dst) !=
                       evhtp_json_vtype_array)) {
        return -1;
    }

    if (!evhtp_array_append(dst->array, src, 1,
                            (void (*))evhtp_json_free)) {
        return -1;
    }
...
j_array_index_(evhtp_json * array, int offset) {
    ...
    return (evhtp_json *)evhtp_array_get_at_index(list, offset);
}
...
static int
j_addbuf_vprintf_(struct __jbuf * jbuf,
                  const char    * fmt,
                  va_list         ap) {
...
static int
j_addbuf_printf_(struct __jbuf * jbuf,
                 const char * fmt, ...) {
    va_list ap;
    int     sres;
    ...
    va_start(ap, fmt);
    {
        sres = j_addbuf_vprintf_(jbuf, fmt, ap);
    }
    va_end(ap);
...
j_number_to_buffer_(evhtp_json * json, struct __jbuf * jbuf) {
    if (evhtp_likely(j_type_(json) != evhtp_json_vtype_number)) {
        ...
        return j_addbuf_number_(jbuf, json->number);
    }
}

static int
j_escape_string_(const char    * str,
                 size_t          len,
                 struct __jbuf * jbuf) {
    unsigned char ch;
    size_t        i;
...
j_string_to_buffer_(evhtp_json * json, struct __jbuf * jbuf) {
    ...
    if (jbuf->escape == true) {
        if (evhtp_unlikely(j_escape_string_(
                               str,
                               json->slen,
                               jbuf) == -1)) {
            return -1;
...
j_null_to_buffer_(evhtp_json * json, struct __jbuf * jbuf) {
    ...
    return j_addbuf_printf_(jbuf, "null");
}

static int
j_to_buffer_(evhtp_json * json, struct __jbuf * jbuf) {
    switch (j_type_(json)) {
...
j_array_to_buffer_(evhtp_json * json, struct __jbuf * jbuf) {
    evhtp_array      * array;
    evhtp_array_elem * elem;
    evhtp_array_elem * temp;
    ...
    for (elem = evhtp_array_first(array); elem; elem = temp) {
        evhtp_json * val;

        val = (evhtp_json *)evhtp_array_elem_data(elem);
        evhtp_assert(val != NULL);

        if (evhtp_unlikely(j_to_buffer_(val, jbuf) == -1)) {
            return -1;
        }

        if ((temp = evhtp_array_next(elem))) {
            if (evhtp_unlikely(
                    j_addbuf_(jbuf, ",", 1) == -1)) {
                return -1;
            }
...
j_object_to_buffer_(evhtp_json * json, struct __jbuf * jbuf) {
    evhtp_keyval     * object;
    evhtp_keyval_ent * ent;
    evhtp_keyval_ent * temp;
    ...
    for (ent = evhtp_keyval_first(object); ent; ent = temp) {
        const char * key;
        evhtp_json * val;

        key = evhtp_keyval_ent_key(ent);
        evhtp_assert(key != NULL);

        val = (evhtp_json *)evhtp_keyval_ent_val(ent);
        evhtp_assert(val != NULL);
        ...
        if (evhtp_unlikely(
                j_addbuf_(
                    jbuf, key,
                    evhtp_keyval_ent_get_klen(ent)) == -1)) {
            return -1;
        }
        ...
        if (evhtp_unlikely(
                j_to_buffer_(val, jbuf) == -1)) {
            return -1;
        }

        if ((temp = evhtp_keyval_next(ent))) {
            if (evhtp_unlikely(
                    j_addbuf_(jbuf, ",", 1) == -1)) {
                return -1;
            }
...
static int
j_number_compare_(evhtp_json            * j1,
                  evhtp_json            * j2,
    ...
    if (evhtp_json_get_number(j1) !=
        evhtp_json_get_number(j2)) {
        return -1;
    }
...
static int
j_array_compare_(evhtp_json            * j1,
                 evhtp_json            * j2,
                 evhtp_json_key_filtercb cb) {
    evhtp_array      * j1_array;
    evhtp_array      * j2_array;
    evhtp_array_elem * elem;
    evhtp_array_elem * temp;
    int                idx;
    ...
    for (elem = evhtp_array_first(j1_array); elem; elem = temp) {
        evhtp_json * j1_val;
        evhtp_json * j2_val;

        j1_val = (evhtp_json *)evhtp_array_elem_data(elem);
        j2_val = (evhtp_json *)evhtp_array_get_at_index(j2_array, idx);
...
static int
j_object_compare_(evhtp_json            * j1,
                  evhtp_json            * j2,
                  evhtp_json_key_filtercb cb) {
    evhtp_keyval     * j1_map;
    evhtp_keyval     * j2_map;
    ...
    for (ent = evhtp_keyval_first(j1_map); ent; ent = temp) {
        const char * key;
        evhtp_json * j1_val;
        evhtp_json * j2_val;

        if (!(key = evhtp_keyval_ent_key(ent))) {
            return -1;
        }

        if (!(j1_val = (evhtp_json *)evhtp_keyval_ent_val(ent))) {
            return -1;
        }
        ...
        if (cb && (cb)(key, j1_val) == 1) {
             ...
             */
            temp = evhtp_keyval_next(ent);
            continue;
        }

        if (!(j2_val = (evhtp_json *)evhtp_keyval_find(j2_map, key))) {
            return -1;
        }
...
static int
j_string_compare_(evhtp_json            * j1,
                  evhtp_json            * j2,
                  evhtp_json_key_filtercb cb) {
    const char * j1_str;
    const char * j2_str;
...
static int
j_boolean_compare_(evhtp_json            * j1,
                   evhtp_json            * j2,
                   evhtp_json_key_filtercb cb) {
    if (!j1 || !j2) {
        return -1;
    ...
    if (evhtp_json_get_boolean(j1) !=
        evhtp_json_get_boolean(j2)) {
        return -1;
    }
...
j_compare_(evhtp_json            * j1,
    ...
    if (evhtp_json_get_size(j1) !=
        evhtp_json_get_size(j2)) {
        return -1;
    }
...
evhtp_json_get_size(evhtp_json * js) {
    ...
    switch (j_type_(js)) {
        ...
        case evhtp_json_vtype_array:
            return evhtp_array_size(js->array);
        case evhtp_json_vtype_object:
            return evhtp_keyval_get_size(js->object);
        default:
            return 0;
    }
...
evhtp_json *
evhtp_json_parse_string(const char * data,
                        size_t       len,
                        size_t     * n_read) {
    unsigned char ch;
    size_t        i;
...
evhtp_json *
evhtp_json_parse_key(const char * data,
                     size_t       len,
                     size_t     * n_read) {
...
evhtp_json *
evhtp_json_parse_number(const char * data,
                        size_t       len,
                        size_t     * n_read) {
    unsigned char ch;
    char          buf[len];
...
evhtp_json *
evhtp_json_parse_boolean(const char * data,
                         size_t       len,
                         size_t     * n_read) {
    evhtp_json * js;
...
evhtp_json *
evhtp_json_parse_null(const char * data,
                      size_t       len,
                      size_t     * n_read) {
    if (len < 4) {
        return NULL;
...
evhtp_json *
evhtp_json_parse_value(const char * data,
                       size_t       len,
                       size_t     * n_read) {
    if (data == NULL || len == 0) {
        /* *n_read = 0; */
    ...
    switch (data[0]) {
        case '"':
            return evhtp_json_parse_string(data,
                                           len, n_read);
        case '{':
            return evhtp_json_parse_object(data,
                                           len, n_read);
        case '[':
            return evhtp_json_parse_array(data,
                                          len, n_read);
        default:
            if (isdigit(data[0])) {
                return evhtp_json_parse_number(
                    data,
                    len,
                    n_read);
            }
            ...
            switch (*data) {
                case 't':
                case 'f':
                    return evhtp_json_parse_boolean(
                        data,
                        len,
                        n_read);
                case 'n':
                    return evhtp_json_parse_null(
                        data,
                        len,
                        n_read);
            }
    ...
    return NULL;
}     /* evhtp_json_parse_value */

evhtp_json *
evhtp_json_parse_array(const char * data,
                       size_t       len,
                       size_t     * n_read) {
    unsigned char     ch;
...
evhtp_json *
evhtp_json_parse_object(const char * data,
                        size_t       len,
                        size_t     * n_read) {
    unsigned char     ch;
    unsigned char     end_ch;
...
evhtp_json *
evhtp_json_parse_buf(const char * data,
                     size_t       len,
                     size_t     * n_read) {
    unsigned char ch;
    size_t        b_read;
...
evhtp_json *
evhtp_json_parse_file(const char * filename,
                      size_t     * bytes_read) {
...
evhtp_json_free(evhtp_json * js) {
    ...
    switch (j_type_(js)) {
        case evhtp_json_vtype_string:
            ...
            break;
        case evhtp_json_vtype_object:
            evhtp_safe_free(js->object, evhtp_keyval_free);
            break;
        case evhtp_json_vtype_array:
            evhtp_safe_free(js->array, evhtp_array_free);
            break;
        default:
            break;
    }
...
evhtp_json *
evhtp_json_get_array_index(evhtp_json * array,
                           int          offset) {
...
evhtp_json *
evhtp_json_path_get(evhtp_json * js,
                    const char * path) {
        ...
        switch (state) {
            case path_state_reading_key:
                switch (ch) {
                    ...
                    case '.':
                        if (!(object =
                                  evhtp_json_get_object(prev))) {
                            return NULL;
                        }

                        if (!(prev =
                                  evhtp_keyval_find(object, buf))) {
                            return NULL;
                        }
...
evhtp_json *
evhtp_json_object_new(void) {
    return j_object_new_();
}

evhtp_json *
evhtp_json_array_new(void) {
    return j_array_new_();
}
...
evhtp_json *
evhtp_json_string_len_new(const char * str, size_t size) {
...
ssize_t
evhtp_json_to_buffer(evhtp_json * json,
                     char       * buf,
                     size_t       buf_len) {
    struct __jbuf jbuf = {
        .buf     = buf,
